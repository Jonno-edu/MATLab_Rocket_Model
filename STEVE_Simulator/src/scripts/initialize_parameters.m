...
Actuators.Nozzle.NaturalFreq = 1500;                % wn_act [rad/s]
...
dataFilePath = fullfile(projectRoot, 'data', 'aero', 'STeVe V1 No Fins.xlsx');
...
if ~exist('STeVeV1NoFins', 'var')
    opts = detectImportOptions(dataFilePath, "Sheet", "Mass Properties");
    opts.DataRange = "A2:F12602";
    opts.VariableNames = ["Time", "Mass", "COM_Z", "MOIx_Z", "MOIy_Z", "MOIz_Z"];
    STeVeV1NoFins = readtable(dataFilePath, opts);
...
if ~exist('STeVeV1NoFinsS2', 'var')
    ...
    % Import the data
    STeVeV1NoFinsS2 = readtable(dataFilePath, opts, "UseExcel", false);
    
    % Import the high angle of attack data if needed
    opts.Sheet = "Aero Properties 15deg";
    opts.DataRange = "A2:O81";
    STeVeV1NoFinsS3 = readtable(dataFilePath, opts, "UseExcel", false);
end
