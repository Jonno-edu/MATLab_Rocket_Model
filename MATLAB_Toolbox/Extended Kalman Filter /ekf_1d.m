...
true_accel_n(t > 15 & t <= 50 + t_pad) = 5;
true_accel_n(t > 70 + t_pad & t <= 80 + t_pad) = -2;
...
perfect_accel_data_x =  true_accel_n.*cos(true_pitch) + true_accel_d.*sin(true_pitch) - g.*sin(true_pitch);
perfect_accel_data_z = -true_accel_n.*sin(true_pitch) + true_accel_d.*cos(true_pitch) - g.*cos(true_pitch);
...
x_hat = zeros(8, 1);
P     = eye(8) * 1;
P(7,7) = (0.5)^2;  % larger initial var for b_ax
P(8,8) = (0.5)^2;
...
for k = 1:N
    ...
    % Bias-corrected IMU
    axb = accel_data_x(k) - b_ax;  % body x specific force
    azb = accel_data_z(k) - b_az;  % body z specific force
    qw  = gyro_data(k)     - b_w;  % body y angular rate (pitch)

    % Precompute
    st = sin(theta); ct = cos(theta);

    % Process model (NED with gravity positive Down)
    x_hat(1) = p_n + v_n * dt;
    x_hat(2) = p_d + v_d * dt;
    x_hat(3) = v_n + (axb*ct - azb*st) * dt;
    x_hat(4) = v_d + (axb*st + azb*ct + g) * dt;
    x_hat(5) = theta + qw * dt;
    % Bias states are random-walk
...
%% 5. Plot Results
% Figure 1: North Position
figure('Name', 'North Position Analysis');
subplot(2,1,1);
hold on; grid on;
plot(t, true_pos_n, 'g', 'LineWidth', 2);
plot(t, x_hat_hist(:,1), 'b--');
plot(t, gps_pos_n_data, 'ro', 'MarkerSize', 4);
legend('True', 'EKF Estimate', 'GPS Measurement');
title('North Position Comparison');
ylabel('Position (m)');
subplot(2,1,2);
hold on; grid on;
plot(t, x_hat_hist(:,1) - true_pos_n, 'r');
plot(t, gps_pos_n_data - true_pos_n, 'mo', 'MarkerSize', 4);
...
% Figure 3: North Velocity
figure('Name', 'North Velocity Analysis');
subplot(2,1,1);
hold on; grid on;
plot(t, true_vel_n, 'g', 'LineWidth', 2);
plot(t, x_hat_hist(:,3), 'b--');
plot(t, gps_vel_n_data, 'ro', 'MarkerSize', 4);
legend('True', 'EKF Estimate', 'GPS Measurement');
title('North Velocity Comparison');
ylabel('Velocity (m/s)');
subplot(2,1,2);
hold on; grid on;
plot(t, x_hat_hist(:,3) - true_vel_n, 'r');
plot(t, gps_vel_n_data - true_vel_n, 'mo', 'MarkerSize', 4);
legend('EKF Error', 'GPS Error');
