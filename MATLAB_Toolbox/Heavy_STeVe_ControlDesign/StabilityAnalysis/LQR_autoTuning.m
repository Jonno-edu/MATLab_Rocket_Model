...
% Pre-allocate a grid to store the gain margin
    ...
    for j = 1:length(k_q_sweep)
        ...
        if all(real(dynamic_poles) < 0)
            try
                ...
                continue; % Skip if margin calculation fails
            end
        end
    end
end
...
%% 5. Plot the Stability Map
figure;
contourf(k_alpha_sweep, k_q_sweep, gm_map, 20, 'LineColor', 'none');
colorbar;
hold on;
% Overlay a contour line for the 6 dB robustness boundary
