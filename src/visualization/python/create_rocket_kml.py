...
data = pd.read_csv('../../../data/results/rocket_trajectory_100fps.csv')
# Column order in new file format: time_s, pos_x_m, pos_y_m, pos_z_m, pitch_rad, ...
time = data['time_s'].values
x = data['pos_x_m'].values
y = data['pos_y_m'].values
z = data['pos_z_m'].values  # Z is already positive in the CSV
theta = data['pitch_rad'].values
...
# Save the KML file
kml.save("../../../output/kml/rocket_trajectory_static.kml")

print(f"""
Static KML file has been created successfully!
Output saved to: ../../../output/kml/rocket_trajectory_static.kml
